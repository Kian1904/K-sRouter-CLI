...
async function _handleCommand(rawInput) {
  ...
  switch (command) {
    ...
    case '/context': {
      if (args === 'clear') {
        const ctxClear = host.getContext();
        if (ctxClear.activePath) {
          host.openPath(ctxClear.activePath);
        }
        console.log(`${C_GREEN}✓ Context cleared — semua file dihapus.${C_RESET}\n`);
        break;
      }
    ...
    case '/unread': {
      if (!args) {
        console.log(`${C_YELLOW}Usage: /unread [filename]  — hapus file dari context${C_RESET}\n`);
        break;
      }
      const ctxNow = host.getContext();
      const found  = ctxNow.files.find(f => f.filename === args || f.filename.includes(args));
      if (!found) {
        console.log(`${C_RED}File "${args}" tidak ada di context.${C_RESET}\n`);
        break;
      }
      const others = ctxNow.files.filter(f => f.filename !== found.filename);
      // Re-open folder resets loadedFiles, then re-read remaining files
      if (ctxNow.activePath) host.openPath(ctxNow.activePath);
      for (const f of others) { host.readFile(f.filename); }
      console.log(`${C_GREEN}✓ "${found.filename}" dihapus dari context.${C_RESET}\n`);
      break;
    }
    ...
    case '/chat': {
      if (args === 'clear') {
        conversationHistory = [];
        console.log(`${C_GREEN}✓ Chat history cleared.${C_RESET}\n`);
        break;
      }
      console.log(`${C_YELLOW}Usage: /chat clear${C_RESET}\n`);
      break;
    }
