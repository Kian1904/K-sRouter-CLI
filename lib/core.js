...
async function _executeTargetRequest(backendUrl, providerId, messages, token) {
  ...
  logger.info(`→ ${providerId} · request sent`);[cite: 1]
  ...
  try {
    ...
    circuit.recordSuccess(providerId);[cite: 1]
    logger.ok(`← ${providerId} · ${duration}ms`);[cite: 1]
    ...
    if (err.name === 'AbortError') {
      errMsg = `timeout ${TIMEOUT_MS}ms`;[cite: 1]
    }

    // Gagal: Hukum provider dan laporkan ke sirkuit breaker
    circuit.recordFailure(providerId);[cite: 1]
    logger.error(`← ${providerId} · ${errMsg}`);[cite: 1]
    throw err;
  }
}
...
export async function sendChatRequest(backendUrl, messages) {
  ...
  const activeProvider = state.activeProvider;[cite: 1]

  if (!token) {
    throw new Error('Akses ditolak: Token pendamping keamanan belum di-set.');
  }

  // Skenario Jalur 1: User mengunci satu provider secara manual via perintah /use[cite: 1]
  if (activeProvider !== 'auto') {[cite: 1]
    if (circuit.isTripped(activeProvider)) {[cite: 1]
  ...
  // Skenario Jalur 2: Sistem "auto" berjalan murni menggunakan loop cascade otomatis[cite: 1]
  const cascadeQueue = providers.getCascadeOrder(false); // Ambil jalur pipa utama[cite: 1]
  
  for (let i = 0; i < cascadeQueue.length; i++) {
    const target = cascadeQueue[i];

    // Lewati provider jika statusnya sedang terisolasi eror (Tripped)[cite: 1]
    if (circuit.isTripped(target.id)) {[cite: 1]
      continue;
    }
    ...
    try {
      ...
      // Jika ini bukan ujung antrean pipa, cetak tanda beralih otomatis ke cadangan[cite: 1]
      if (i < cascadeQueue.length - 1) {
        // Ambil ID provider berikutnya untuk keperluan log kronologis
        const nextTarget = cascadeQueue[i + 1];
        logger.info(`→ ${nextTarget.id} · fallback cascade`);[cite: 1]
      }
    }
  }

  // Skenario Jalur 3: Jalur utama habis, aktifkan barikade terakhir (Backup Redflag Providers)[cite: 1]
  logger.warn('Seluruh pipa utama lumpuh. Mengaktifkan rute darurat backup redflag...');
  const backupQueue = providers.getCascadeOrder(true).filter(p => p.backup);[cite: 1]
...
/**
 * Melakukan pengecekan kesehatan server backend via HTTP Ping non-token[cite: 1]
 */
export async function checkServerStatus(backendUrl) {
  const targetUrl = `${backendUrl.replace(/\/$/, '')}/api/status`;[cite: 1]
  const startTime = Date.now();

  try {
    const res = await _fetchWithTimeout(targetUrl, { method: 'GET' }, 15000);
    const duration = Date.now() - startTime;
