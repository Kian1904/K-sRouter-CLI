 ...
 */
async function _executeTargetRequest(backendUrl, providerId, messages, token, effort) {
  ...
  try {
    const response = await _fetchWithTimeout(targetUrl, {
      ...
      body: JSON.stringify({ provider: providerId, messages: messages, effort: effort || 'medium' })
    }, TIMEOUT_MS);
...
export async function sendChatRequest(backendUrl, messages) {
  ...
  // Inject memory context ke system message
  ...
  try {
    ...
    // Memory inject gagal — lanjut tanpa memory, jangan crash
  ...
  if (activeProvider !== 'auto') {
    ...
    return await _executeTargetRequest(backendUrl, activeProvider, enrichedMessages, token, effort);
  }
  ...
  for (let i = 0; i < cascadeQueue.length; i++) {
    ...
    try {
      return await _executeTargetRequest(backendUrl, target.id, enrichedMessages, token, effort);
    } catch (e) {
