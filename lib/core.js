...
export async function sendChatRequest(backendUrl, messages) {
  ...
  if (activeProvider !== 'auto') {
    ...
    return await _executeTargetRequest(backendUrl, activeProvider, messages, token, effort);
  }
  ...
  for (let i = 0; i < cascadeQueue.length; i++) {
    ...
    try {
      return await _executeTargetRequest(backendUrl, target.id, messages, token, effort);
    } catch (e) {
